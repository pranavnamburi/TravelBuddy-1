...
@WebServlet(name="FriendPoolRetreiveUserMadeRequests", value="/friend_pool_retreive_user_made_requests")
public class Friend_PoolRetreiveUserMadeRequests extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            PreparedStatement statement = connection.prepareStatement("SELECT receiver_id, status from bt_base.friendpoolrequests where sender_id=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            ...
            if(noOfRequests == 1 && resultSet.getString("status").equals("accepted")) {
                req.getSession().setAttribute("friendPoolRequestAcceptance", true);
            }else {
                req.getSession().setAttribute("friendPoolRequestAcceptance", false);
            }
            // Closing the statement
            statement.close();
            // Closing the connection to the database
            connection.close();
            req.getSession().setAttribute("friendPoolUserMadeRequests", userMadeRequests);
